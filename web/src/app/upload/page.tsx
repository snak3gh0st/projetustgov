  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        ...
        <h1 className="text-2xl font-bold text-gray-900 mt-4">Importar Planilha</h1>
          ...
          <div className="bg-white border border-gray-200 shadow-sm rounded-2xl p-8">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
              <div className="bg-gray-50 rounded-xl p-4 border border-gray-200">
                ...
                <p className="text-2xl font-bold text-gray-900">{results.totalRows.toLocaleString('pt-BR')}</p>
              </div>
            ...
            {(results.auto_distributed > 0 || results.enriched > 0 || results.existing_clients > 0) && (
              <div className="grid grid-cols-3 gap-4 mb-6">
                ...
                {results.existing_clients > 0 && (
                  <div className="bg-gray-50 rounded-xl p-4 border border-orange-200">
                    <p className="text-xs text-gray-500">Ja sao Clientes</p>
                    <p className="text-xl font-bold text-orange-600">{results.existing_clients.toLocaleString('pt-BR')}</p>
            ...
            {results.api_enriched > 0 && (
              <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
                <div className="bg-gray-50 rounded-xl p-4 border border-purple-200">
                  <p className="text-xs text-gray-500">Receita Federal (API)</p>
                  <p className="text-xl font-bold text-purple-600">{results.api_enriched}</p>
                  <p className="text-xs text-gray-600 mt-1">
                    {results.api_phone_added} tel, {results.api_email_added} email
                  </p>
                </div>
                {results.api_remaining > 0 && (
                  <div className="bg-gray-50 rounded-xl p-4 border border-yellow-200">
            ...
            {results.sheets.length > 1 && (
              <div>
                <h3 className="text-sm font-semibold text-gray-900 mb-3">Detalhes por aba</h3>
                    ...
                    <tbody>
                      {results.sheets.map((s, i) => (
                        <tr key={i} className="border-t border-gray-200">
                          <td className="px-4 py-2 text-gray-900">{s.sheet}</td>
                          <td className="px-4 py-2 text-gray-400">{s.vendedor || '-'}</td>
                          <td className="px-4 py-2 text-[#0072F7]">{s.rows}</td>
                          <td className="px-4 py-2 text-amber-600">{s.duplicates}</td>
