...
export default function DistribuirPage() {
  ...
  async function handleAssign() {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json()
        setToast(`${data.assigned_count ?? selectedLeadIds.size} leads atribuídos com sucesso!`)
        setSelectedLeadIds(new Set())
        fetchLeads()
        fetchVendedores()
  ...
  if (userRole !== 'gestor') {
    return <div className="flex items-center justify-center py-20 text-gray-500">Verificando permissões...</div>
  }
  ...
  return (
    <div className="space-y-6 max-w-[1400px]">
      <div>
        ...
        <p className="text-sm text-gray-400 mt-1">Atribua leads não atribuídos aos vendedores</p>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/5 bg-sigma-navy-light">
                ...
                <th className="px-3 py-3 text-left text-xs font-medium text-gray-400 uppercase">Município</th>
              </tr>
            </thead>
            <tbody>
      ...
      <div className="sticky bottom-4 flex items-center justify-between rounded-xl border border-white/10 bg-gray-950/90 backdrop-blur-md px-4 py-3">
        <span className="text-sm text-gray-400">
          {selectedLeadIds.size > 0
            ...
            : `${filteredLeads.length} leads não atribuídos`
          }
        </span>
        <div className="flex items-center gap-3">
          ...
          >
            {assigning ? 'Atribuindo...' : `Atribuir ${selectedLeadIds.size} leads`}
          </button>
        </div>
      </div>
      ...
      {/* Toast */}
      {toast && (
        <div className="fixed bottom-6 right-6 bg-cyan-500/20 border border-cyan-500/30 text-cyan-300 px-4 py-2 rounded-lg text-sm backdrop-blur-md animate-pulse">
          {toast}
        </div>
      )}
    </div>
